...
//var array = JSON.stringify(arrayTest);
//array = JSON.parse(array);

var arrSkills = _.map(array, 'skills');
var allSkills = _.flatten(arrSkills);
var noRepeatSkills = _.uniq(allSkills);
var noRepeatSkills2 = _.sortedUniq(allSkills); //почему не работает???
var sortSkills = _.sortBy(noRepeatSkills, function(o){ return _.camelCase(o)});//упорядочивание по алфавиту
console.log("Отсортированный массив skills", sortSkills);

var arrName = _.map(array, 'name'); //массив имен до сортировки, для сравнения.
var arrSortNameByFriends = _.sortBy(array,['friends', 'name']);
var sortName = _.map(arrSortNameByFriends, 'name');
console.log("Отсортированный по друзьям массив", sortName);

var arrFriends = _.map(array, 'friends');
var arrFriendsName = _.flatten(arrFriends); //разбираем массивы
arrFriendsName = _.map(arrFriendsName, 'name'); //выборка из объектов по ключу
var noRepeatNameOfFriends = _.uniq(arrFriendsName); //уникальность
console.log("Не повторяющиеся друзья всех пользователей", noRepeatNameOfFriends);